module.exports = function init(site) {
  ...
  site.on('please add user', (u) => {
    ...
    security.getUser({ email: u.email }, (err, u2) => {
  ...
  security.users.push({
    ...
    roles: ['*'],
    $permissions: ["*"],
    profile: { name: site.options.security.admin.email }
  })
  ...
  security.reset = function () {
    ...
    security.loadAllUsers((err, users) => {
      ...
      for (var i = 0; i < site.options.security.users.length; i++) {
        ...
        doc = security.handleUser(doc)
        doc.profile = { name: site.options.security.admin.email }
  ...
  security.addUser = function (user, callback) {
    ...
    security.isUserExists(user, function (err, u) {
      ...
      if (u) {
        ...
        user.permissions = user.permissions || []
        user.roles = user.roles || []
        user.added_user_info = security.getUserFinger({$req : $req , $res : $res})
  ...
  security.updateUser = function (user, callback) {
    ...
    if (user.id) {
      ...
      callback({ message: 'no id or email' })
      return
    }
      ...
      function (err, result) {
        if (err) {
          ...
          for (let i = 0; i < security.users.length; i++) {
            ...
            if (element.id == id) {
              element = Object.assign(element, security.handleUser(user))
  ...
  security.addUserPermission = function (id, permission, callback) {
    callback = callback || function () { }
    let user = security.getUser({ id: id }, function (err, user) {
      if (user) {
