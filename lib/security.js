  ...
  security.login = function (user, callback) {
    ...
    for (var i = 0; i < security.users.length; i++) {
      ...
      if (
        ...
        (!_user.key && _user.mobile === user.mobile && _user.password === user.password) ||
        (!_user.key && _user.userName === user.userName && _user.password === user.password) ||
    ...
    ____0.$users.findOne(
      {
        where: {
          $or: [{ email: user.email }, { mobile: user.mobile }, { userName: user.userName }],
