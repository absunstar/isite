module.exports = function init(site) {
 ...
 const  $user = site.connectCollection({
    ...
    db: site.options.security.dbName
  })

  const security = function(){}
  security.users = []

  security.loadUsers = function(callback) {
    $user.findMany(
      {
        limit: 10000,
        select: {_id : 1 , email : 1 , password : 1 , permissions : 1}
      },
      function(err, docs) {
        if (!docs) {
          docs = []
  ...
  security.getUser = function(id, callback) {
    if (id) {
    ...
    $user.updateOne(
      ...
      function(err, result) {
        if (err) {
          ...
          callback(err, result)
          security.loadUsers(function(err, users) {
            if (users) {
