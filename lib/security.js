module.exports = function init(site) {
   ...
   site.$users = site.connectCollection({
    collection: site.options.security.users_collection,
    db: site.options.security.db
  })

   site.$roles = site.connectCollection({
    collection: site.options.security.roles_collection,
    db: site.options.security.db
  })


  site.$users.deleteDuplicate({
    email: 1
  }, (err, result) => {
    site.$users.createUnique({
      email: 1
    })
  })

  site.$roles.deleteDuplicate({
    name: 1
  }, (err, result) => {
    site.$roles.createUnique({
      name: 1
    })
  })
  ...
  site.on('please add user', (u, callback) => {
  ...
  security.updateRole = security.editeRole = function (role, callback) {
    callback = callback || function () {}
    site.$roles.update(role, (err, result) => {
      if (!err) {
  ...
  security.deleteRole = security.removeRole = function (role, callback) {
    callback = callback || function () {}
    site.$roles.delete({where : {id : role.id}}, (err, result) => {
      if (!err) {
      ...
      for (var i = 0; i < security.users.length; i++) {
        ...
        if (_user.email == user.email && _user.password == user.password) {
          ...
          site.call('user login', {
            db: site.$users.db,
            collection: site.$users.collection,
            doc: _user,
            $res: $res,
            $req: $req
          })
