module.exports = function init(___0) {
  ...
  security.users = [];
  ___0.options.security.keys.push(___0.options.security.key)
  ___0.options.security.keys.forEach((key) => {
    security.users.push({
      id: key,
      key: key,
      is_admin: !0,
  ...
  ___0.options.security.users.forEach((user) => {
    user.key = user.key || ___0.options.security.key;
  ...
  security.getUsers = function (options, callback) {
    callback = callback || function () {};
    ___0.$users.findMany(options, function (err, docs, count) {
      if (!err && docs) {
        docs.forEach((doc) => {
  ...
  security.getUser = function (_user, callback) {
    ...
    for (let i = 0; i < security.users.length; i++) {
      ...
      if ((_user.id && user.id === _user.id) || (!user.key && _user.email && user.email === _user.email) || (!user.key && _user.key && user.key === _user.key)) {
        user.$memory = !0;
  ...
  security.login = function (user, callback) {
    ...
    for (var i = 0; i < security.users.length; i++) {
      ...
      if (
        ...
        (_user.key && _user.key === ___0.md5(user.email + user.password))
      ) {
        if ($req) {
  ...
  security.register = function (user, callback) {
    ...
    security.isUserExists(user, function (err, u) {
      if (u) {
        ...
        ___0.$users.insertOne(user, function (err, doc) {
          if (doc) {
            doc = security.handleUser(doc);
