module.exports = function init(____0) {
    const existingUsers = ____0.$users;
    const existingRoles = ____0.$roles;
    ____0.$users = existingUsers && typeof existingUsers.findMany === 'function' && typeof existingUsers.createIndex === 'function'
        ? existingUsers
        : ____0.connectCollection({ collection: ____0.options.security.users_collection, db: ____0.options.security.db });
    ____0.$roles = existingRoles && typeof existingRoles.findMany === 'function' && typeof existingRoles.createIndex === 'function'
        ? existingRoles
        : ____0.connectCollection({ collection: ____0.options.security.roles_collection, db: ____0.options.security.db });

    /** Email Must Not Duplicate but can be null */

    // These Mongo maintenance operations were always asynchronous and no
    // legacy API could observe them as completed when init() returned. Start
    // them on the next event-loop turn so HTTP listen can win the startup
    // critical path without changing the operations, callbacks, or indexes.
    const maintenance = setImmediate(() => {
    ...
    if (maintenance && maintenance.unref) maintenance.unref();
    ...
    security.users = [];
    security.userIndexes = { id: new Map(), _id: new Map(), email: new Map(), username: new Map(), mobile: new Map(), key: new Map() };
    // External/session user providers let applications keep iSite's native
    // req.session.user login contract while loading users from non-$users stores
    // such as JSON files. The default path remains unchanged when no source is set.
    security.userProviders = new Map();
    security.registerUserProvider = function (name, provider) {
        name = String(name || '').trim();
        if (!name || (typeof provider !== 'function' && typeof provider?.getUser !== 'function')) {
            throw new TypeError('A user provider name and function/getUser handler are required');
        }
        security.userProviders.set(name, provider);
        return provider;
    };
    security.unregisterUserProvider = function (name) {
        return security.userProviders.delete(String(name || '').trim());
    };
    security.getSessionUser = function (session, callback) {
        callback = callback || function () {};
        session = session || {};
        const source = String(session.user_source || session.userSource || '').trim();
        const userId = session.user_id != null ? session.user_id : session.user?.id;
        if (!source) {
            return security.getUser({ id: userId }, callback);
        }
        const provider = security.userProviders.get(source);
        if (!provider) {
            const error = new Error('Session user provider not found: ' + source);
            error.code = 'USER_PROVIDER_NOT_FOUND';
            callback(error, null);
            return;
        }
        let finished = false;
        const done = function (err, user) {
            if (finished) return;
            finished = true;
            if (!err && user) user = security.handleUser(user);
            callback(err, user || null);
        };
        try {
            const handler = typeof provider === 'function' ? provider : provider.getUser.bind(provider);
            const result = handler({ id: userId, source, session }, done);
            if (result && typeof result.then === 'function') result.then((user) => done(null, user)).catch(done);
            else if (result !== undefined && handler.length < 2) done(null, result);
        } catch (error) {
            done(error, null);
        }
    };
    security.setSessionUser = function (req, user, options = {}) {
        if (!req || !req.session || !user) return null;
        user = security.handleUser(user);
        req.session.user = user;
        req.session.user_id = user.id;
        if (options.source) req.session.user_source = String(options.source);
        else { delete req.session.user_source; delete req.session.userSource; }
        if (options.authMethod) req.session.user_auth_method = String(options.authMethod);
        req.session.$userLoadedAt = Date.now();
        if (typeof req.session.$save === 'function') req.session.$save();
        return user;
    };
    security.clearSessionUser = function (req) {
        if (!req || !req.session) return;
        req.session.user = null;
        req.session.user_id = null;
        req.session.user_source = null;
        req.session.userSource = null;
        req.session.user_auth_method = null;
        req.session.$userLoadedAt = 0;
        if (typeof req.session.$save === 'function') req.session.$save();
    };
    security.roleByName = new Map();
    security.permissionByName = new Map();
    security.rebuildRoleIndexes = function () {
        security.roleByName.clear();
        security.permissionByName.clear();
        for (const role of security.roles) if (role && role.name) security.roleByName.set(role.name, role);
        for (const permission of security.permissions) if (permission && permission.name) security.permissionByName.set(permission.name, permission);
    };
    security.indexUser = function (user) {
        if (!user) return user;
        const idx = security.userIndexes;
        if (user.id != null) idx.id.set(String(user.id), user);
        if (user._id != null) idx._id.set(String(user._id), user);
        if (user.email && !user.key) idx.email.set(String(user.email).trim().toLowerCase(), user);
        if (user.username && !user.key) idx.username.set(String(user.username).trim().toLowerCase(), user);
        if (user.mobile && !user.key) idx.mobile.set(String(user.mobile).trim().toLowerCase(), user);
        if (user.key) idx.key.set(String(user.key), user);
        return user;
    };
    security.rebuildUserIndexes = function () {
        for (const map of Object.values(security.userIndexes)) map.clear();
        security.users.forEach(security.indexUser);
    };
    security.cacheUser = function (user) {
        if (!user) return user;
        const existing = user.id != null ? security.userIndexes.id.get(String(user.id)) : null;
        if (!existing) security.users.push(user);
        else if (existing !== user) Object.assign(existing, user), user = existing;
        return security.indexUser(user);
    };
    security.findCachedUser = function (query) {
        if (!query) return null;
        const idx = security.userIndexes;
        if (query.id != null) return idx.id.get(String(query.id)) || null;
        if (query._id != null) return idx._id.get(String(query._id)) || null;
        if (query.email) {
            const email = String(query.email).trim().toLowerCase();
            return idx.email.get(email) || idx.key.get(____0.x0md50x(email)) || null;
        }
        if (query.username) return idx.username.get(String(query.username).trim().toLowerCase()) || null;
        if (query.mobile) return idx.mobile.get(String(query.mobile).trim().toLowerCase()) || null;
        if (query.key) return idx.key.get(String(query.key)) || null;
        return null;
    };
    ...
    security.updateRole = security.editeRole = function (role, callback) {
        ...
        ____0.$roles.update(role, (err, result) => {
            if (!err) {
                const indexedRole = (role.id != null ? security.roleIndexes.id.get(String(role.id)) : null) || (role.name ? security.roleIndexes.name.get(String(role.name)) : null);
                const index = indexedRole ? security.roles.indexOf(indexedRole) : -1;
                if (index !== -1) security.roles[index] = { ...security.roles[index], ...role };
                else security.roles.push(role);
                security.rebuildRoleIndexes();
            }
            callback(err, result);
        });
    ...
    security.deleteRole = security.removeRole = function (role, callback) {
        ...
        ____0.$roles.delete(
            ...
            (err, result) => {
                ...
                if (!err) security.rebuildRoleIndexes();
                callback(err, result);
            },
        );
    };
    ...
    // v13: cache the immutable timestamp for today's legacy `getDate()` value.
    // Each call still receives a fresh Date instance, preserving identity/mutability semantics.
    security._fingerDateCache = { value: 0, expiresAt: 0 };
    security._getFingerDate = function () {
        const now = Date.now();
        let cache = security._fingerDateCache;
    ...
    security.handleUser = function (_user) {
        ...
        const permissionInfoNames = new Set();

        const addPermission = function (name) {
            if (!name || permissionNames.has(name)) return;
            permissionNames.add(name);
            _user.$permissions.push(name);
            const info = security.permissionByName.get(name);
            if (info && !permissionInfoNames.has(name)) {
                permissionInfoNames.add(name);
                _user.$permissions_info.push(info);
            }
        };
        const addRolePermissions = function (roleRef) {
            if (!roleRef) return;
            const roleName = typeof roleRef === 'string' ? roleRef : roleRef.name;
            if (roleName === '*') { addPermission('*'); return; }
            const role = security.roleByName.get(roleName);
            if (!role || !Array.isArray(role.permissions)) return;
            for (const permission of role.permissions) addPermission(typeof permission === 'string' ? permission : permission?.name);
        };

        for (const permission of _user.permissions) addPermission(typeof permission === 'string' ? permission : permission?.name);
        if (_user.role) addRolePermissions(_user.role);
        for (const role of _user.roles) {
            if (role && typeof role === 'object') delete role.permissions;
    ...
    security.getUser = function (_user, callback) {
        callback = callback || function () {};
        let cached = security.findCachedUser(_user);
        if (cached) {
            cached.$memory = !0;
            cached = security.handleUser(cached);
            security.indexUser(cached);
            callback(null, cached);
            return;
        }
        ____0.$users.findOne({ where: _user, select: {} }, function (err, doc) {
            if (!err && doc) {
                doc = security.handleUser(doc);
                security.cacheUser(doc);
            }
            callback(err, doc);
        });
    };
        ...
        ____0.$users.update(
            ...
            function (err, result) {
                ...
                if (!err && result && result.doc) {
                    const cachedUser = security.userIndexes.id.get(String(result.doc.id));
                    if (cachedUser) Object.assign(cachedUser, result.doc);
                    else security.cacheUser(result.doc);
                    security.rebuildUserIndexes();
                    if (____0.sessions && ____0.sessions.invalidateUser) ____0.sessions.invalidateUser(result.doc.id);
                }
            },
        );
    };
                ...
                if (err) {
                    ...
                    for (let i = 0; i < security.users.length; i++) {
                        ...
                        if (element.id == user.id) {
                            ...
                            security.rebuildUserIndexes();
                            if (____0.sessions && ____0.sessions.invalidateUser) ____0.sessions.invalidateUser(user.id);
                            break;
                        }
                    }
                    callback(err, result);
