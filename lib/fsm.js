...
module.exports = function init(_f_) {
  ...
  fsm.getContent = function(name , callback) {
    ...
    let path = _f_.path.join(_f_.dir , 'site_files', _f_.path.extname(name).replace('.', ''), name)

    if (!_f_.isFileExistsSync(path)) {
      let arr = name.split('/')
      if (arr.length === 2) {
        path = _f_.path.join(_f_.path.dirname(_f_.dir), 'apps', arr[0], 'site_files', _f_.path.extname(arr[1]).replace('.', ''), arr[1])
      } else if (arr.length === 3) {
        path = _f_.path.join(_f_.path.dirname(_f_.dir), 'apps', arr[0], 'site_files', _f_.path.extname(arr[2]).replace('.', ''), arr[1], arr[2])
      }
    }
      ...
      if (arr.length > 1) {
        _f_.apps.forEach(ap => {
          if (arr.length === 2 && ap.name == arr[0]) {
            path = _f_.path.join(ap.path, 'site_files', _f_.path.extname(arr[1]).replace('.', ''), arr[1])
          } else if (arr.length === 2 && ap.name2 == arr[0]) {
            path = _f_.path.join(ap.path, 'site_files', _f_.path.extname(arr[1]).replace('.', ''), arr[1])
          } else if (arr.length === 3 && ap.name == arr[0]) {
            path = _f_.path.join(ap.path, 'site_files', _f_.path.extname(arr[2]).replace('.', ''), arr[1], arr[2])
          }
        })
      }
    }
