module.exports = function init(___0) {
  ...
  fsm.createDir = fsm.mkDir = (dir, callback) => {
    ...
    fs.exists(___0.path.dirname(dir), ok => {
      if (ok) {
        fs.mkdir(dir, err => {
          callback(err, dir)
        })
      } else {
        let parentDir = ___0.path.dirname(dir)
        fsm.mkDir(parentDir, () => {
          fs.mkdir(dir, err => {
            callback(err, dir)
          })
  ...
  fsm.readFileSync = function (path, callback) {
    ...
    let encode = "utf8"
    encode = ___0.fn.getFileEncode(path)
    if (fsm.isFileExistsSync(path)) {
  ...
  fsm.getContent = function (name, callback) {
    callback = callback || function () {}
    let extname = ___0.path.extname(name).replace('.', '')
    if (extname == 'png' || extname == 'jpg' || extname == 'jpeg' || extname == 'bmp' || extname == 'ico') {
      extname = 'images'
    }
    let path = ___0.path.join(___0.dir, extname, name)

    if (!___0.isFileExistsSync(path)) {
      let arr = name.split('/')
      if (arr.length === 2) {
        let extname = ___0.path.extname(arr[1]).replace('.', '')
        if (extname == 'png' || extname == 'jpg' || extname == 'jpeg' || extname == 'bmp' || extname == 'ico') {
          extname = 'images'
        }
        path = ___0.path.join(___0.path.dirname(___0.dir), 'apps', arr[0], 'site_files', extname, arr[1])
      } else if (arr.length === 3) {
        let extname = ___0.path.extname(arr[2]).replace('.', '')
        if (extname == 'png' || extname == 'jpg' || extname == 'jpeg' || extname == 'bmp' || extname == 'ico') {
          extname = 'images'
        }
        path = ___0.path.join(___0.path.dirname(___0.dir), 'apps', arr[0], 'site_files', extname, arr[1], arr[2])
      }
    }
      ...
      if (arr.length > 1) {
        ___0.apps.forEach(ap => {
          if (arr.length === 2 && ap.name == arr[0]) {
            ...
            path = ___0.path.join(ap.path, 'site_files',extname, arr[1])
            ...
            path = ___0.path.join(ap.path, 'site_files',extname, arr[1], arr[2])
          }
        })
      }
    }
  ...
  fsm.downloadFile = function (path, req, res) {
    fsm.stat(path, (err, stats) => {
      if (!err) {
        res.writeHead(200, {
          "Content-Type": ___0.fn.contentType(path),
          "Content-Length": stats.size,
          "Content-Disposition": "attachment; filename=" + (___0.path.basename(path))
        })
