module.exports = function init(____0) {
  ...
  fsm.readFile = function (path, callback) {
    return fsm.readFileSync(path, callback);
  };
  ...
  fsm.readFileSync = function (path, callback) {
    ...
    if (index !== -1) {
      ...
      if (callback) {
        callback(null, fsm.list[index].content, fsm.list[index]);
      } else {
    ...
    if (fsm.isFileExistsSync(path)) {
      let file = {
        path: path,
        content: fs.readFileSync(path, encode),
        count: 1,
      ...
      let file = {
        path: path,
        content: fs.readFileSync(path2, encode),
        count: 1,
        stat: fsm.statSync(path),
        encode: encode,
        time: new Date().getTime(),
