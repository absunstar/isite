module.exports = function init(req, res, ____0, route) {
    ...
    ____0._parserTokenCache = ____0._parserTokenCache || new Map();
    const tokenCache = ____0._parserTokenCache;
    const TOKEN_CACHE_MAX = 1000;
    function compileTokenTemplate(txt) {
        let compiled = tokenCache.get(txt);
        if (compiled) {
            tokenCache.delete(txt);
            tokenCache.set(txt, compiled);
            return compiled;
        }
        const re = /##(var|user|site|req|session|json|setting|params|query|data|word)\.([^#]*?)##/g;
        const parts = [];
        let last = 0;
        let match;
        while ((match = re.exec(txt))) {
            if (match.index > last) parts.push(txt.slice(last, match.index));
            parts.push({ type: match[1], value: match[2], raw: match[0] });
            last = match.index + match[0].length;
        }
        if (last < txt.length) parts.push(txt.slice(last));
        compiled = parts.some((p) => typeof p === 'object') ? parts : null;
        tokenCache.set(txt, compiled);
        if (tokenCache.size > TOKEN_CACHE_MAX) tokenCache.delete(tokenCache.keys().next().value);
        return compiled;
    }
    function renderToken(type, value, raw) {
        switch (type) {
            case 'var': return renderVar(value);
            case 'user': return renderUser(value);
            case 'site': return render_site(value);
            case 'req': return renderRequest(value);
            case 'session': return renderSession(value);
            case 'json': return renderJson(value);
            case 'setting': return renderSetting(value);
            case 'params': return renderParam(value);
            case 'query': return renderQuery(value);
            case 'data': return renderData(value);
            case 'word': return renderWord(value);
            default: return raw;
        }
    }
    parser.handleMatches = function (txt) {
        ...
        let passes = 0;
        while (txt.includes('##') && passes++ < 10) {
            const compiled = compileTokenTemplate(txt);
            if (!compiled) break;
            let changed = false;
            const out = compiled.map((part) => {
                if (typeof part === 'string') return part;
                const value = renderToken(part.type, part.value, part.raw);
                if (String(value ?? '') !== part.raw) changed = true;
                return value ?? '';
            }).join('');
            txt = out;
            if (!changed) break;
