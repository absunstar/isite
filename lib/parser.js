module.exports = function init(req, res, ___0, route) {
  let parser = {};

  function renderVar(v) {
    if (v && v == '*') {
      return JSON.stringify(___0.var);
    }
    return ___0.var(v);
  }
  ...
  function renderJson(name) {
    return ___0.readFileSync(route.parserDir + '/json/' + name + '.json');
  }

  function renderWord(name) {
    let w = ___0.words.get(name);
    if (w.done) {
      return w.word[req.session.lang] || name;
    } else {
      return name;
    }
  }
  ...
  function renderSetting(v) {
    if (v && v == '*') {
      return JSON.stringify(___0.setting.list);
    }
    return ___0.setting.get(v).value;
  }

  function getContent(name) {
    let path = ___0.path.join(route.parserDir, ___0.path.extname(name).replace('.', ''), name);

    if (!___0.isFileExistsSync(path)) {
      let arr = name.split('/');
      if (arr.length === 2) {
        path = ___0.path.join(___0.path.dirname(route.parserDir), 'apps', arr[0], 'site_files', ___0.path.extname(arr[1]).replace('.', ''), arr[1]);
      } else if (arr.length === 3) {
        path = ___0.path.join(___0.path.dirname(route.parserDir), 'apps', arr[0], 'site_files', ___0.path.extname(arr[2]).replace('.', ''), arr[1], arr[2]);
      }
    }
    ...
    if (!___0.isFileExistsSync(path)) {
      let arr = name.split('/');
      if (arr.length > 1) {
        ___0.apps.forEach((ap) => {
          if (arr.length === 2 && ap.name == arr[0]) {
            path = ___0.path.join(ap.path, 'site_files', ___0.path.extname(arr[1]).replace('.', ''), arr[1]);
          } else if (arr.length === 2 && ap.name2 == arr[0]) {
            path = ___0.path.join(ap.path, 'site_files', ___0.path.extname(arr[1]).replace('.', ''), arr[1]);
          } else if (arr.length === 3 && ap.name == arr[0]) {
            path = ___0.path.join(ap.path, 'site_files', ___0.path.extname(arr[2]).replace('.', ''), arr[1], arr[2]);
          }
        });
      }
    }
    ...
    if (!___0.isFileExistsSync(path)) {
      ___0.log(path, 'PATH NOT EXISTS');
      return '';
    }

    if (name.endsWith('.html')) {
      let txt = ___0.readFileSync(path);
      let $ = ___0.$.load(txt);
      $ = renderHtml($);
      return $.html();
  ...
  function renderHtml($ , log) {
    $('[x-setting]').each(function (i, elem) {
      if (!___0.setting.get($(this).attr('x-setting')).value) {
        $(this).remove();
      }
    });

    $('[x-permission]').each(function (i, elem) {
      if (!___0.security.isUserHasPermission(req, res, $(this).attr('x-permission'))) {
        $(this).remove();
      }
    });

    $('[x-role]').each(function (i, elem) {
      if (!___0.security.isUserHasRole(req, res, $(this).attr('x-role'))) {
        $(this).remove();
      }
    });

    $('[x-permissions]').each(function (i, elem) {
      if (!___0.security.isUserHasPermissions(req, res, $(this).attr('x-permissions'))) {
        $(this).remove();
      }
    });

    $('[x-roles]').each(function (i, elem) {
      if (!___0.security.isUserHasRoles(req, res, $(this).attr('x-roles'))) {
        $(this).remove();
      }
    });
