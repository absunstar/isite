module.exports = function init(req, res, ____0, route) {
  ...
  function handleXList($, el, data) {
    let property = $(el).attr('x-list');
    $(el).removeAttr('x-list');
    let list = null;
    let html2 = '';
    if (property) {
      if (property == '*') {
    ...
    if (Array.isArray(list)) {
      let matches = $.html(el).match(/##item.*?##/g);
      list.forEach((item, i) => {
        $(el).attr('x-item', i);
        let _html = $.html(el);
        if (matches) {
          for (let i = 0; i < matches.length; i++) {
            let p = matches[i].replace('##item.', '').replace('##', '').split('.');
            let v = null;
            if (p.length > 0) {
              v = item[p[0]];
            }
  ...
  function renderHtml($, log) {
    ...
    $('[x-list]').each(function (i, elem) {
      $(handleXList($, elem, req.data)).insertAfter($(this));
      $(this).remove();
    });
    return $;
  }
