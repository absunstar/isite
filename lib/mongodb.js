module.exports = function init(____0) {
  ...
  ____0.on('[close-database]', (args, callback) => {
    ...
    if (_mongo.dbList.length === 0) {
      callback();
      return;
    }

    _mongo.closeDbBusy = !0;
    console.log('Closing mongodb Connection Count : ' + _mongo.dbList.length);
    for (let i = 0; i < _mongo.dbList.length; i++) {
      console.log('Closing Database : ' + _mongo.dbList[i].name);
      _mongo.dbList[i].client.close();
    }
  ...
  _mongo.connectDB = async function (name, callback) {
    ...
    for (let i = 0; i < _mongo.dbList.length; i++) {
      if (_mongo.dbList[i].name === name) {
        callback(null, _mongo.dbList[i].db);
        _mongo.connectDBBusy = !1;
        return;
      }
    }
  ...
  _mongo.connectCollection = function (options, callback) {
    ...
    let index = _mongo.collectionList.findIndex((c) => c.name == name);
    if (index !== -1) {
      callback(null, _mongo.collectionList[index].collection);
    } else {
      _mongo.connectDB(options.dbName, function (err, db) {
        if (!err) {
          _mongo.collectionList.push({
            name: name,
            collection: db.collection(name),
          });
          callback(null, _mongo.collectionList[_mongo.collectionList.findIndex((c) => c.name == name)].collection);
        } else {
          callback(err, null);
        }
      });
  ...
  _mongo.delete = _mongo.deleteMany = function (obj, callback) {
    ...
    _mongo.connectCollection(obj.dbName, function (err, collection) {
      if (!err) {
        if (obj.collectionName === undefined) {
          obj.collectionName = ____0.options.mongodb.collection;
        }
