      ...
      for (let i = 0; i < _mongo.connections.length; i++) {
        if (_mongo.connections[i].name === name) {
          if (_mongo.connections[i].connected) {
            ...
            callback({ message: 'db connection lost' }, null)
            _mongo.connectDBBusy = false
            return
          }
      ...
      _mongo.client.connect(db_url, function (err, db) {
        ...
        if (!err) {
          ...
          if (site.options.mongodb.identity.enabled) {
            $collection_indexed = site.connectCollection({
              collection: 'collection_indexed',
              db: site.options.mongodb.prefix.db + name
            })

            $collection_indexed.deleteDuplicate({
              'name': 1
            }, (err, result) => {
              $collection_indexed.createUnique({
                'name': 1
              })
            })
          }
