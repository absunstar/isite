module.exports = function init(site) {
    ...
    _mongo.connectDB = function (name, callback) {
        if(name === undefined){
            name = site.options.mongodb.default.db
        }
        if (site.options.mongodb.enabled) {
            _mongo.client.connect(_mongo.connection + '/' + site.options.mongodb.prefix.db + name + '?maxPoolSize=0', function (err, db) {
                callback(err, db)
            })
        } else {
            callback({
        ...
        _mongo.connectDB(obj.dbName, function (err, db) {
            if (!err) {
                if(obj.collectionName === undefined){
                    obj.collectionName = site.options.mongodb.default.collection
                }
    ...
    _mongo.find = _mongo.findMany = function (obj, callback) {
        _mongo.connectDB(obj.dbName, function (err, db) {
            if (!err) {
                ...
                db.collection(site.options.mongodb.prefix.collection + obj.collectionName).find(obj.where, obj.select).sort(obj.sort || {}).limit(obj.limit || 10).toArray(function (err, docs) {
                    db.close()
                    if (!err) {
                        callback(null, docs)
                    } else {
