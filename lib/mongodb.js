module.exports = function init(site) {
  ...
  let url = site.options.mongodb.host + ":" + site.options.mongodb.port
  if (site.options.mongodb.userName && site.options.mongodb.password) {
    url = encodeURIComponent(site.options.mongodb.userName) + ":" + encodeURIComponent(site.options.mongodb.password) + "@" + site.options.mongodb.host + ":" + site.options.mongodb.port
  }
  ...
  _mongo.connectDB = function (name, callback) {
    ...
    if (site.options.mongodb.enabled) {
      ...
      let db_url = _mongo.connection + "/" + site.options.mongodb.prefix.db + name + "?authSource=admin"
      _mongo.client.connect(db_url, function (err, db) {
  ...
  _mongo.dropCollection = function (obj, callback) {
    _mongo.connectDB(obj.dbName, function (err, db) {
      if (!err) {
        ...
        db.collection(site.options.mongodb.prefix.collection + obj.collectionName).drop(function (err, delOK) {
  ...
  _mongo.find = _mongo.findMany = function (obj, callback) {
    _mongo.connectDB(obj.dbName, function (err, db) {
      if (!err) {
        ...
        db.collection(site.options.mongodb.prefix.collection + obj.collectionName).count(obj.where, function (err, count) {
          ...
          if (count > 0) {
            ...
            if (obj.skip) {
              result = result.skip(obj.skip)
            }

            if (obj.sort) {
              result = result.sort(obj.sort)
            }

            result.limit(Math.floor(Math.random() * (obj.limit || site.options.mongodb.limit)))
