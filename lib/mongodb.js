module.exports = function init(_m_) {
  ...
  _mongo.connectDB = function (name, callback) {
    ...
    if (_m_.options.mongodb.enabled) {
      ...
      let db_url = _mongo.connection + "/" + _m_.options.mongodb.prefix.db + name + "?authSource=admin"
  ...
  _mongo.aggregate = function (obj, callback) {
    _mongo.connectDB(obj.dbName, function (err, db) {
      if (!err) {
        ...
        db.collection(_m_.options.mongodb.prefix.collection + obj.collectionName).aggregate(obj.arr, function (err, result) {
  ...
  _mongo.findOne = function (obj, callback) {
    ...
    _mongo.connectDB(obj.dbName, function (err, db) {
      if (!err) {
        ...
        db.collection(_m_.options.mongodb.prefix.collection + obj.collectionName).findOne(obj.where, obj.select, function (err, doc) {
  ...
  _mongo.find = _mongo.findMany = function (obj, callback) {
    _mongo.connectDB(obj.dbName, function (err, db) {
      if (!err) {
        ...
        db.collection(_m_.options.mongodb.prefix.collection + obj.collectionName).count(obj.where, function (err, count) {
          ...
          if (count > 0) {
            let result = db
              .collection(_m_.options.mongodb.prefix.collection + obj.collectionName)
            ...
            result.limit(Math.floor(Math.random() * (obj.limit || _m_.options.mongodb.limit)))
