  ...
  _mongo.aggregate = function (obj, callback) {
    _mongo.connectDB(obj.dbName, function (err, db) {
      if (!err) {
        ...
        db.collection(____0.options.mongodb.prefix.collection + obj.collectionName).aggregate(obj.arr).toArray(function (err, docs) {
    ...
    _mongo.connectDB(obj.dbName, function (err, db) {
      if (!err) {
        ...
        _mongo.findOne({
          ...
          if (!err) {
            db.collection(____0.options.mongodb.prefix.collection + obj.collectionName).updateOne(obj.where, $update,
              function (err, result) {
  ...
  _mongo.update = _mongo.updateMany = function (obj, callback) {
    _mongo.connectDB(obj.dbName, function (err, db) {
      if (!err) {
        ...
        db.collection(____0.options.mongodb.prefix.collection + obj.collectionName).updateMany(obj.where, $update,
          function (err, result) {
