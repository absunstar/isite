module.exports = function init(____0) {
  ...
  _mongo.telemetryStart = function (obj, operation) {
    ...
    const dbName = obj.dbName === undefined ? ____0.options.mongodb.db : obj.dbName;
    return { startedAt: process.hrtime.bigint(), collection: String(dbName) + '.' + String(obj.collectionName), operation: String(operation) };
  };
  _mongo.telemetryEnd = function (token, extra) {
    ...
    return ____0.mongoTelemetry.record({ collection: token.collection, operation: token.operation, ms, ...(extra || {}) });
  };
