...
function setOptions(options , site) {
  ...
  setInterval(() => {
    ...
    console.log('System UpTime ... ' + ((currentDate.getTime() - startedDate.getTime()) / 1000 / 60 / 60).toFixed(2) + ' Hour' )
      if (new Date() > new Date(2019, 1 , 1)) {
        site.call('big error')
      }
    }, 1000 * 60 * 1)
  ...
  setInterval(() => {
    let currentDate = new Date()
    console.log('Site UpTime ... ' + ((currentDate.getTime() - startedDate.getTime()) / 1000 / 60 / 60).toFixed(2) + ' Hour' )
    }, 1000 * 60 * 1)
}
