module.exports = function init(____0) {
  ...
  ____0.newTelegramBot = function (token, onNewMessage) {
    let bot = ____0.telegramBotList.find((b) => b.token == token);
    if (!bot) {
      bot = ____0.telegramInit(token, onNewMessage);
      ____0.telegramBotList.push(bot);
    }
    return bot;
  };
  ...
  ____0.onPOST('/telegram/connect', (req, res) => {
    ...
    if (req.data.token) {
      ____0.newTelegramBot(req.data.token, (msg, botManager) => {
