module.exports = function init(____0) {
  ...
  ____0.onPOST('/telegram/connect', (req, res) => {
    ...
    if (req.data.token) {
      let bot = parent.telegramBotList.find((b) => b.token == req.data.token)?.bot;
      if (!bot) {
        bot = ____0.telegramInit(req.data.token, (msg) => {
          console.log(msg);
        });
        parent.telegramBotList.push({
  ...
  ____0.onPOST('/telegram/disconnect', (req, res) => {
    ...
    if (req.data.token) {
      let index = parent.telegramBotList.findIndex((b) => b.token == req.data.token);
      if (index !== -1) {
        parent.telegramBotList = parent.telegramBotList.filter((b) => b.token !== req.data.token);
        response.done = true;
