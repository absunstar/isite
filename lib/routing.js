module.exports = function init(____0) {
  ...
  _0xrrxo.endResponse = function (req, res) {
    ...
    if (route.shared && !____0.sharedList.some((s) => s.filePath == response.filePath)) {
  ...
  _0xrrxo.defaultCallback = function (req, res) {
    let route = req.route;
    if ((response = ____0.sharedList.find((s) => s.filePath == route.path))) {
      res.headers = response.headers;
      res.code = response.code;
      res.setHeader('x-shared', ____0.sharedList.length);
  ...
  _0xrrxo.handleServer = async function (req, res) {
    ...
    res.html = res.render = function (file, _data = null, options = {}) {
      ...
      if ((response = ____0.sharedList.find((s) => s.filePath == filePath))) {
        res.headers = response.headers;
        res.code = response.code;
        res.setHeader('x-shared', ____0.sharedList.length);
        return res.end(response.content, response.encode);
      ...
      ____0.fsm.getContent(filePath, (content) => {
        ...
        if (options.shared && !____0.sharedList.some((s) => s.filePath == response.filePath)) {
          ____0.sharedList.push(response);
        }
