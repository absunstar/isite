module.exports = function init(___0) {
  ...
  _0xrrxo.start = function (_ports, callback) {
    ...
    ports.forEach((p, i) => {
      ___0.servers[i] = ___0.http.createServer(_0xrrxo.handleServer);
      ___0.servers[i].listen(p, function () {
        ___0.log('');
        ___0.log('-----------------------------------------');
    ...
    if (___0.options.https.enabled) {
      ...
      ___0.options.https.ports.forEach((p, i) => {
        ___0.servers[i] = ___0.https.createServer(https_options, _0xrrxo.handleServer);
        ___0.servers[i].listen(p, function () {
          ___0.log('');
          ___0.log('-----------------------------------------');
