module.exports = function init(____0) {
    ...
    _0xrrxo.handleServer = async function (req, res) {
        ____0.validateServerRequest(req, res, (req, res) => {
            if (____0.diagnostics) ____0.diagnostics.requestStart(req);
                ...
                if (____0.isFileExistsSync(path)) {
                    ...
                    if (stat && stat.isFile()) {
                        ...
                        if (rangeHeader) {
                            ...
                            const readStream = ____0.fs.createReadStream(path, { start, end });
                            readStream.on('end', function () { readStream.close(); });
                            res.on(____0.strings[10], function () { readStream.close(); });
                            res.writeHead(206, {
