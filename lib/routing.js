module.exports = function init(site) {
  ...
  routing.endResponse = function (req, res) {
    ...
    if (!site.is_safty) {
      if (site.options.help) {
        res.set('help-error-message', 'site not safty')
      }
      res.end()
      return
    }
    ...
    if (typeof route.path == "string" && site.fs.lstatSync(route.path).isDirectory()) {
      ...
      if (!exist) {
        ...
        if (route.name.like('*api*')) {
          console.error('xxx Duplicate Route xxx ' + route.name)
        } else {
          console.warn('>>> Duplicate Route >>> ' + route.name)
        }
      }
  ...
  routing.handleServer = function (req, res) {
      ...
      if (typeof arg1 === 'number') {
        ...
        if (arg1 && res.headers && res.headers['Content-Type'] && (
          ...
          if (req.acceptEncoding.match(/\bdeflate\b/) && typeof arg1 === 'string') {
            ...
            arg1 = site.zlib.deflateSync(new Buffer.from(arg1))
          } else if (req.acceptEncoding.match(/\bgzip\b/) && typeof arg1 === 'string') {
            res.set('Content-Encoding', 'gzip')
            res.set('Vary', 'Accept-Encoding')
            arg1 = site.zlib.gzipSync(new Buffer.from(arg1))
          } else {
            site.log(typeof arg1)
          }
        }
    ...
    for( let i = 0 ; i < routing.list.length && i2 === -1 ; i++){
      ...
      if (req.method.toLowerCase() === routing.list[i].method.toLowerCase() && req.urlParser.pathname === routing.list[i].name) {
