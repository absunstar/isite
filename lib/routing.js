  ...
  routing.handleRoute = async function (req, res, route) {
    ...
    if (route.require.features.length > 0) {
      ...
      if (!ok) {
        ...
        res.status(401);
        return res.render('client-side/require_features.html' , {
    ...
    if (route.require.permissions.length > 0) {
      ...
      if (!ok) {
        ...
        res.status(401);
        return res.render('client-side/require_permissions.html' , {
  ...
  routing.handleServer = async function (req, res) {
    ...
    if (!req.urlParser.pathname.like('*x-api*|*x-css*')) {
      ...
      if (!_r_._is_) {
        ...
        res.status(402);
        return res.render('client-side/require_payments.html' , {
