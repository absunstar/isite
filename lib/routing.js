module.exports = function init(site) {
  ...
  routing.endResponse = function (req, res) {
    ...
    if (route.empty) {
      if (site.options.help) {
        res.set('x-message', 'route is empty')
      }
      res.end()
      return
    }
    ...
    if (req.headers["if-none-match"] == hash) {
      if (site.options.help) {
        res.set('x-message', 'etag matched')
      }
  ...
  routing.defaultCallback = function (req, res) {
    ...
    if (route.cache && route.content) {
      if (site.options.help) {
        res.set("x-content", "From Route Memory")
      }
    ...
    if (!route.path) {
      if (site.options.help) {
        res.set("x-content", "Route Not Set File Path")
      }
      res.code = 200
      routing.endResponse(req, res)
      return
    ...
    if (typeof route.path == "string") {
      site.readFile(route.path, function (err, data, file) {
        if (!err) {
          ...
          if (site.options.help) {
            res.set("x-content", "Route Read File")
          }
          ...
          if (site.options.help) {
            res.set("x-error", "Route Error Read File")
          }
      ...
      site.readFiles(route.path, function (err, data) {
        if (!err) {
          if (site.options.help) {
            res.set("x-content", "Route Read Files")
          }
          ...
          if (site.options.help) {
            res.set("x-error", "Route Error Read Files")
          }
  ...
  routing.start = function (ports, callback) {
    ...
    let app = site.http.createServer(function (req, res) {
      ...
      res.ip = req.ip = req.socket.remoteAddress.replace("::ffff:", "")
      ...
      res.cookie = res.cookies = req.cookie = req.cookies = site.cookie(req, res, site)
      ...
      for (let i = 0; i < routing.list.length; i++) {
        ...
        if (
          ...
          if (!req.method.toLowerCase().like("get") &&
            ...
            req.on("end", function () {
              req.data = req.body = site.querystring.parse(req.body)
            ...
            req.on("end", function () {
              req.data = req.body = site.fromJson(req.body)
              if (site.options.session.enabled) {
                site.session(req, res, site, function (session) {
                  req.session = session
                  r.callback(req, res)
      ...
      if (req.urlParser.pathname == "/") {
        if (site.options.help) {
          res.set("x-message", "unhandled route")
        }
    ...
    for (let i = 0; i < ports.length; i++) {
      app.listen(ports[i], function () {
        console.log("")
        console.log("-----------------------------------------")
        console.log("    " + site.options.name + " Running on Port : " + ports[i])
        console.log("-----------------------------------------")
        console.log("")
