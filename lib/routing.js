module.exports = function init(____0) {
  ...
  _0xrrxo.handleServer = async function (req, res) {
    ...
    if (matched_routes.length > 0) {
      ...
      if (!req.method.toLowerCase().like('get') && req.headers[____0.strings[7]] && req.headers[____0.strings[7]].match(/urlencoded/i)) {
      ...
      } else if (req.method.toLowerCase().contains('post') && req.headers[____0.strings[7]] && req.headers[____0.strings[7]].contains('multipart')) {
        ...
        return;
      } else if (!req.method.toLowerCase().like('get') && req.headers[____0.strings[7]] && req.headers[____0.strings[7]].match(/json/i)) {
        req.on('data', function (data) {
          req.bodyRaw += data;
        });
        req.on('end', function () {
