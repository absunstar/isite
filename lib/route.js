module.exports = function init(site) {
    ...
    function add(r, callback) {
        ...
        if (typeof r == "string") {
            ...
            route.cache = true
            route.map = []
            route.callback = callback || defaultCallback
        } else {
            ...
            route.map = r.map || []
            route.callback = r.callback || defaultCallback
        }
        ...
        if (typeof route.path == 'string' && fs.lstatSync(route.path).isDirectory()) {
            fs.readdir(route.path, (err, files) => {
                files.forEach(file => {
                    var r2 = JSON.parse(JSON.stringify(route));
