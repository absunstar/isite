...
module.exports = function initCoreV3(site) {
    ...
    // ---------------------------------------------------------------------
    // Capabilities: feature-detect once, then let all modules share it.
    // ---------------------------------------------------------------------
    site.capabilities = Object.freeze({
        ...
        workerThreads: (() => { try { require('node:worker_threads'); return true; } catch (_) { return false; } })(),
        brotli: !!site.zlib?.brotliCompress,
        http2: !!site.http2,
    ...
    site.events = {
        ...
        once(name, fn) { emitter.once(name, fn); return () => emitter.off(name, fn); },
        off(name, fn) { emitter.off(name, fn); },
        emit(name, ...args) { return emitter.emit(name, ...args); },
        listenerCount(name) { return emitter.listenerCount(name); },
        removeAll(name) { emitter.removeAllListeners(name); },
    };
    ...
    site.httpCache = {
        etag(value, weak = true) {
            ...
            const hash = crypto.createHash('sha1').update(input).digest('base64url').slice(0, 27);
    ...
    site.profile = function (name, fn) {
        ...
        if (typeof fn !== 'function') return function end() { const ms = performance.now() - start; profileRecord(String(name), ms); return ms; };
        try {
            const result = fn();
            if (result && typeof result.then === 'function') return result.finally(() => profileRecord(String(name), performance.now() - start));
            profileRecord(String(name), performance.now() - start); return result;
        } catch (err) { profileRecord(String(name), performance.now() - start); throw err; }
    };
