module.exports = function init(req, res, ___0, callback) {
  ...
  session.save = function () {
    session.$changed = !0;
    session = ___0.sessions.handle(session);
  };
    ...
    for (var i = 0; i < session.data.length; i++) {
      ...
      if (key.indexOf('.') != -1) {
        if (d.key == key.split('.')[0]) {
          return ___0.fromJson(___0.copy(session.data[i].value))[key.split('.')[1]];
          break;
        }
      } else {
        if (d.key == key) {
          return ___0.copy(session.data[i].value);
          break;
        }
      }
    }
  ...
  function AssignFeatures() {
    req.features = [];
    ___0.options.defaults.features.forEach((f) => {
      req.features.push(f);
    });
    ___0.features.forEach((f) => {
      req.features.push(f.name);
    });
    if (___0.options.dynamic) {
  ...
  if (___0.security && session.user_id) {
    ___0.security.getUser(
      {
        id: session.user_id,
      },
      function (err, user) {
