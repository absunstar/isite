module.exports = function init(req, res, _s_, callback) {
    ...
    if (!session.browserToken) {
        ...
        res.cookie('browser_token', session.browserToken , {expires : 60 * 24 * 365})
    }
    ...
    function AssignFeatures() {
        ...
        if (req.headers['user-agent']) {
            ...
            } else if (req.userAgent.contains('baidu')) {
    ...
    if (_s_.security && session.user_id !== undefined) {
        _s_.security.getUser({id : session.user_id}, function (err, user) {
            session.user = user
            if (session.user) {
