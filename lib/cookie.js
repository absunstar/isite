...
function stringifyCookies(cookies, expires) {
    ...
    return list.join('; ') + ';path=/; expires=' + new Date(new Date().getTime() + (1000 * 60 * expires)).toUTCString();;
}
...
module.exports = function init(req, res, site) {
    ...
    cookie.set = function(key, value) {
        ...
        res.setHeader('Set-Cookie', stringifyCookies(newCookies, site.options.session.timeout));
    }
