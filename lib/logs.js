module.exports = function init(___0) {
  const logs = function () {};
  logs.list = [];
  logs.$collectoin = ___0.connectCollection('app_setting');
  ...
  ___0.on(___0.strings[9], () => {
    ...
    ___0.get('/x-api/logs/:key/:value', (req, res) => {
    ...
    ___0.get('/x-api/logs/:key', (req, res) => {
      res.json({
        value: logs.fn(req.params.key),
      });
    });
    ___0.get('/x-api/logs', (req, res) => {
      res.json(logs.list);
    });

    ___0.post('/api/isite/saved', (req, res) => {
