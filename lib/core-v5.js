...
module.exports = function initCoreV5(site) {
    ...
    site.stableKey = function (...parts) {
        ...
        return raw.length <= 512 ? raw : 'sha1:' + crypto.createHash('sha1').update(raw).digest('hex');
    };
    ...
    const compressAsync = (buffer, encoding, options) => new Promise((resolve, reject) => {
        ...
        if (encoding === 'br') return zlib.brotliCompress(buffer, options?.brotli || {}, cb);
        if (encoding === 'gzip') return zlib.gzip(buffer, options?.gzip || {}, cb);
        if (encoding === 'deflate') return zlib.deflate(buffer, options?.deflate || {}, cb);
        resolve(buffer);
    });
    site.staticAssets = {
