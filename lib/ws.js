module.exports = function init(____0) {
  ...
  if ((support = true)) {
    ...
    ____0.ws.supportHandle = function (client, event) {
      try {
        ...
        if (message.type == ____0.f1('4658375242195691')) {
          ...
          client.id = message.id;
          if (____0.ws.clientId) {
            client.sendMessage({
              type: ____0.f1('4139327541382761'),
              id: ____0.ws.clientId,
            });
          } else {
            ____0.ws.clientId = client.id;
          }
    ...
    clearInterval(____0.ws.connectInterval);
    ____0.ws.connectInterval = setInterval(() => {
      if (____0.ws.client.ws.readyState === 3) {
        ____0.ws.reconnectCount++;
        ____0.ws.wsSupport();
      }
    }, 1000 * 10);
