module.exports = function init(____0) {
                    ...
                    if (pathname === route.options.name) {
                        ...
                        ____0.ws.server.handleUpgrade(request, socket, head, function done(ws) {
                            let client = {
                                ...
                                socket: socket,
                                head: head,
                                ip1: request.headers[____0.strings[6]],
                                ip2: request.connection.remoteAddress,
                                ip:  request.headers[____0.strings[6]] ?? request.connection.remoteAddress ? request.connection.remoteAddress.replace('::ffff:', '') : '0.0.0.0',
    ...
    ____0.ws.onNewSupportedClient = function (client) {
        console.log(`New Supported Client ( ${client.ip1} - ${client.ip2} ) / ${____0.ws.supportedClientList.length}`);
    };
