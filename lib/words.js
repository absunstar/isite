module.exports = function init(___0) {
  ...
  words.list = [];
  words.db_list = [];
  words.$collectoin = ___0.connectCollection('app_setting');
  ...
  ___0.on(___0.strings[9], () => {
    ___0.get({ name: '/x-api/words', require: { permissions: ['login'] } }, (req, res) => {
      res.json({done : !0 , words : [...words.db_list , ...words.list]});
    });

    ___0.post({ name: '/x-api/words', require: { permissions: ['login'] } }, (req, res) => {
      res.json(words.add(req.data.word));
    });

    ___0.get('/x-api/words/get/:name', (req, res) => {
      res.json({
