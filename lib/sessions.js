module.exports = function init(____0) {
  ...
  sessions.handleSessions = function () {
    ...
    if (____0.options.session.storage === 'mongodb') {
      sessions.$collection.deleteAll({ createdTime: { $lt: new Date().getTime() - 1000 * 60 * 60 * 24 * 30 } });
  ...
  sessions.saveAll = function (callback) {
    ...
    sessions.list = sessions.list.filter((s) => s && new Date().getTime() - s.time < 1000 * 60 * 60);
