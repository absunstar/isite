module.exports = function init(___0) {
    ...
    if (___0.options.mongodb.enabled) {
        $sessions = ___0.connectCollection({
            db: ___0.options.session.db,
            collection: ___0.options.session.collection
        })
    }

    ___0.on('[session][update]' , session =>{
    ...
    ___0.on('[session][delete]' , session =>{
    ...
    ___0.on('[session][user][update]' , user =>{
        sessions.list.forEach(s => {
    ...
    sessions.loadAll = function (callback) {
        ...
        if (___0.options.session.storage !== 'mongodb' || ___0.options.mongodb.enabled === !1) {
            callback({
    ...
    sessions.saveAll = function (callback) {
        ...
        if (___0.options.session.timeout === 0) {
            callback({
        ...
        sessions.list = sessions.list.filter(s => {
            ...
            let timeout = 1000 * 60 * ___0.options.session.timeout
    ...
    sessions.handle = function (session, callback) {
        ...
        sessions.list.forEach((s , i) => {
            if (s.accessToken == session.accessToken) {
                ...
                session.lang = session.lang || s.lang || ___0.options.lang
                session.theme = session.theme || s.theme || ___0.options.theme
        ...
        if (!session_exists) {
            session.$new = !0
            session.lang = ___0.options.lang
            session.theme = ___0.options.theme
            session.data = []
    ...
    ___0.get('x-api/sessions' , (req , res)=>{
        res.json({
            done : !0 ,
            list : sessions.list
        })
    ...
    ___0.get('x-api/sessions/save' , (req , res)=>{
        sessions.saveAll()
        res.json({
            done : !0
        })
    })
    ___0.get('x-api/sessions/delete' , (req , res)=>{
        sessions.list = []
