    ...
    $collection.editOne =
      (options, callback, run) => {
        ...
        if (run !== 2 && run !== !0) {
          ...
          if (newOptions.where && typeof newOptions.where === 'string') {
            newOptions.where = {
              _id: $collection.ObjectId(newOptions.where),
            };
          }

          if (newOptions.where && newOptions.where._id && typeof newOptions.where._id === 'string') {
            newOptions.where._id = $collection.ObjectId(newOptions.where._id);
          }
