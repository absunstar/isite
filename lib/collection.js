module.exports = function init(site, option , db) {
  ...
  if (typeof option === "string") {
    ...
    $collection.db = db || site.options.mongodb.db
  } else if (typeof option === "object") {
    $collection.collection = option.collection || site.options.mongodb.collection
    $collection.db = option.db || site.options.mongodb.db
  } else {
    $collection.collection = site.options.mongodb.collection
  ...
  $collection.drop = (callback) => {
    ...
    site.mongodb.dropCollection({
      collectionName: $collection.collection,
      dbName: $collection.db
    }, (err, ok) => {
      if(ok){
        site.mongodb.collections_indexed[$collection.collection].nextID = 1
      }
      callback(err, ok)
    })
  }
  ...
  $collection.insertOne = $collection.insert = $collection.add = $collection.addOne = ($doc, callback) => {
    ...
    if ($doc) {
      ...
      if (doc_info && doc_info.doc) {
        ...
        site.log(doc_info.doc ,  $collection.collection)
