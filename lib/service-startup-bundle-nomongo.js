// GENERATED no-Mongo startup bundle. Original lib modules remain public and authoritative.

// Intentionally no top-level strict mode: legacy modules may rely on sloppy-mode implicit globals.
...
module.exports = function init(____0) {
    ...
    _0xrrxo.handleServer = async function (req, res) {
        ...
        ____0.validateServerRequest(req, res, (req, res) => {
            ...
            // Resolve each translation key once per request. Large pages can contain
            // thousands of word tokens and often repeat navigation/footer labels.
            const requestWordCache = new Map();
            req.word = function (name) {
                if (requestWordCache.has(name)) return requestWordCache.get(name);
                const w = ____0.word(name);
                let value;
                if (!w.hostList) {
                    value = w[req.session.language.id] || name;
                } else {
                    const w2 = w.hostList.find((h) => req.host.like(h.name));
                    value = w2 ? (w2[req.session.language.id] || name) : (w[req.session.language.id] || name);
                }
                requestWordCache.set(name, value);
                return value;
            };
    ...
    _0xrrxo.start = function (_ports, callback) {
        ...
        const primaryReady = function () {
            ...
            if (!primaryReadyDone && primaryReadyCount >= ports.length) {
                ...
                // v26: keep advanced additive core layers off the listen critical
                // path, then warm them on the first event-loop turn after the
                // readiness callback. Immediate access from the callback still
                // activates them synchronously through the compatibility getters.
                if (typeof ____0._initAdvancedCore === 'function') {
                    defer(() => { try { ____0._initAdvancedCore(); } catch (_) {} });
                }
...
module.exports = function init(req, res, ____0, route) {
    ...
    // Cache resolved words for the lifetime of this request. A page can reference the
    // same key many times (navigation, footer, repeated cards). Resolving it once also
    // avoids repeating host/language lookup work for large translated pages.
    const renderedWordCache = new Map();
    function renderWord(name) {
        if (renderedWordCache.has(name)) return renderedWordCache.get(name);
        const value = req.word(name);
        renderedWordCache.set(name, value);
        return value;
    }
    ...
    // v33: single-pass token renderer. The previous implementation called
    // String#replace once for every token and recursively rescanned the complete
    // document. With thousands of translated words that becomes effectively O(n*k).
    // This implementation scans the string once per expansion pass and replaces every
    // supported token through one RegExp callback. Extra passes are only needed when a
    // rendered value itself introduces another template token.
    const templateTokenPattern = /##(var|user|site|req|session|json|setting|params|query|data|word)\.([\s\S]*?)##/g;
    parser.handleMatches = function (txt) {
        if (typeof txt !== 'string' || txt.indexOf('##') === -1) return txt;

        let output = txt;
        // Keep legacy nested-token behavior, but cap pathological recursive templates.
        for (let pass = 0; pass < 16 && output.indexOf('##') !== -1; pass++) {
            let handled = false;
            templateTokenPattern.lastIndex = 0;
            const next = output.replace(templateTokenPattern, (full, kind, name) => {
                handled = true;
                switch (kind) {
                    case 'var': return renderVar(name);
                    case 'user': return renderUser(name);
                    case 'site': return render_site(name);
                    case 'req': return renderRequest(name);
                    case 'session': return renderSession(name);
                    case 'json': return renderJson(name);
                    case 'setting': return renderSetting(name);
                    case 'params': return renderParam(name);
                    case 'query': return renderQuery(name);
                    case 'data': return renderData(name);
                    case 'word': return renderWord(name);
                    default: return full;
                }
            });
            output = next;
...
module.exports = { data, fsm, routing, vars, words, storage, logs, ws, wsClient, email, integrated, browser, helper, pdf, app, evalMod, proxy, sessions, cookie, session, parser, dashboard };
