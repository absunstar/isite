(function (window, document, undefined, $) {
  ...
  site.showTabContent = function (e, tabContentSelector) {
    ...
    document
      .querySelectorAll(tabContentSelector + ".tab-content")
      .forEach((el) => {
  ...
  site.toHtmlTable = function (obj) {
    ...
    if (site.typeOf(obj) == "Object") {
      ...
      for (
        let index = 0;
        index < Object.getOwnPropertyNames(obj).length;
        index++
      ) {
      ...
      vList.forEach((vl) => {
        ...
        if (vl === "r") {
          if (
            ...
            (!el.value || el.value.like("*undefined*"))
          ) {
          ...
          if (
            ...
            (!el.value || el.value.length > length)
          ) {
            site.vControles.push({
              el: el,
              border: border,
  ...
  site.stringfiy = function (_num, lang) {
    ...
    if (s2) {
      s +=
        site.strings["space"][lang] +
        site.strings["and"][lang] +
        site.strings["space"][lang] +
        s2;
    }
    return s;
