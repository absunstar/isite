module.exports = function (site) {
  ...
  site.post({ name: "/x-api/upload/image", public: true }, (req, res) => {
    site.createDir(
      site.options.upload_dir + "/" + req.headers["folder"],
      () => {
        site.createDir(
          site.options.upload_dir + "/" + req.headers["folder"] + "/images",
          () => {
            let response = {
              image: {},
              done: !0,
  ...
  site.get(
    { name: "/x-api/image/:category/:name", public: true },
    (req, res) => {
  ...
  site.post({ name: "/x-api/upload/file", public: true }, (req, res) => {
    site.createDir(
      ...
      () => {
        site.createDir(
          site.options.upload_dir + "/" + req.headers["folder"] + "/files",
          () => {
            let response = {
              file: {},
              done: !0,
  ...
  site.get(
    { name: "/x-api/file/:category/:name", public: true },
    (req, res) => {
      res.download(
        site.options.upload_dir +
          "/" +
